...
namespace {
//*
template <class Graph> class tarjan_scc_visitor : public default_dfs_visitor<Graph> {
  ComponentMap &comp;
  long &c;
...
public:
  tarjan_scc_visitor(ComponentMap &comp_map, long &cref, AbstractMap<typename Graph::Vertex> &rootmap,
                     AbstractMap<long> &d, std::stack<typename Graph::Vertex> &_s, long &dref)
      : comp(comp_map), c(cref), root(rootmap), discover_time(d), s(_s), dfs_time(dref) {}

  inline void discoverVertex(typename Graph::Vertex v, const Graph &) {
    root.put(v, v);
  ...
  inline void finishVertex(typename Graph::Vertex v, const Graph &g) {
...
private:
  template <typename Vertex> Vertex min_discover_time(Vertex u, Vertex v) {
    return discover_time.get(u) < discover_time.get(v) ? u : v;
  }
};

template <class Graph> long strong_components_impl(Graph &g, ComponentMap &comp) {
  long total = 0;
  std::stack<typename Graph::Vertex> s;
  ...
  ColorMap color(g);
  depth_first_search(g, vis, g.null_vertex(), color);  // null_vertex is a hack, that means: start from all vertices
  return total;
}

} // namespace

template <class Graph> long strong_components(Graph &g, ComponentMap &comp) { return strong_components_impl(g, comp); }

template <class InGraph, class OutGraph> long strong_components_graph(InGraph &inGraph, OutGraph &outGraph) {

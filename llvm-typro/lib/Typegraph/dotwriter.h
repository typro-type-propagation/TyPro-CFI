...
template <class Graph, class Vertex> struct default_node_filter {
  bool operator()(Graph &graph, Vertex &v) { return true; }
};

template <class Graph, class NodeWriter, class EdgeWriter, class NodeFilter>
inline void dot_write_graph(std::ostream &out, Graph &graph, NodeWriter nodeWriter, EdgeWriter edgeWriter,
                            NodeFilter filter) {
  out << "digraph pdg {\n";
...
template <class Graph, class NodeWriter, class EdgeWriter, class NodeFilter>
inline void dot_debug_graph(Graph &graph, NodeWriter nodeWriter, EdgeWriter edgeWriter, NodeFilter filter) {
  std::ofstream fs("/tmp/debuggraph.dot");
  dot_write_graph(fs, graph, nodeWriter, edgeWriter, filter);
  fs.close();
  system("dot -Txlib /tmp/debuggraph.dot");
}

template <class Graph, class Vertex, class NodeWriter, class EdgeWriter, class NodeFilter>
inline void dot_bfs_write_graph(std::ostream &out, Graph &graph, std::vector<Vertex> &sources, NodeWriter nodeWriter,
                                EdgeWriter edgeWriter, NodeFilter &filter, const std::string legend = "") {
  out << "digraph pdg {\n";
  out << legend << "\n";

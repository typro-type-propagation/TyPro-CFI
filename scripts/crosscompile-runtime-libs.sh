...
make_rt_libs () {
	...
	#LD="$CLANG_BIN/ld.lld"
	export CFLAGS="$FLAGS --sysroot $SYSROOT -isystem $SYSROOT/usr/lib/gcc/$1/10/include -isystem $SYSROOT/usr/include/$1"
	export CXXFLAGS="$FLAGS --sysroot $SYSROOT -isystem $SYSROOT/usr/include/c++/10 -isystem $SYSROOT/usr/include/$1/c++/10 -isystem $SYSROOT/usr/include/c++/10/backward -isystem $SYSROOT/usr/lib/gcc/$1/10/include -isystem $SYSROOT/usr/include/$1"
